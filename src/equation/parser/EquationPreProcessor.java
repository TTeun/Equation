...
import equation.parser.exception.BadBracketsException;
...
public class EquationPreProcessor {
    ...
    static public String preProcessEquationString(@NotNull String equationString) throws EquationException {
        System.out.print("\033[1;37m");
        System.out.print("Pre-processing: \t\t");

        if (equationString.length() == 0) {
            throw new EquationException("Empty equation string");
        }
        equationString = equationString.trim();
        if (!Character.isAlphabetic(equationString.charAt(0))) {
            throw new BadHeaderException(new EquationException("No function name declared"));
        }
        ...
        equationString = addDepthToBrackets(equationString, 0, '(', ')', '[', ']');
        equationString = addDepthToBrackets(equationString, 0, '{', '}', '(', ')');
    ...
    static private String addDepthToBrackets(String equationString, int depth, char bo, char bc, char newBo, char newBc) {
        Pattern openBracketPattern = Pattern.compile("\\" + bo);
        ...
        if (matcher.find()) {
            ...
            equationString = equationString.substring(0, indexOfFunctionOpenBracket - 1) + newBo + "_" + depth +
                    addDepthToBrackets(equationString.substring(matcher.end(), indexOfFunctionCloseBracket), depth + 1, bo, bc, newBo, newBc) +
                    newBc + "_" + depth + equationString.substring(indexOfFunctionCloseBracket + 1);

            return addDepthToBrackets(equationString, 0, bo, bc, newBo, newBc);
        }

        return equationString;
    }

//    static private String addDepthToFunctionCalls(String equationString, int depth) {
...
//            }
//            assert (indexOfFunctionCloseBracket != -1);
//            equationString = equationString.substring(0, indexOfFunctionOpenBracket - 1) + "(_" + depth +
//                    addDepthToBrackets(equationString.substring(matcher.end(), indexOfFunctionCloseBracket), depth + 1) +
//                    ")_" + depth + equationString.substring(indexOfFunctionCloseBracket + 1);
//
//            return addDepthToBrackets(equationString, 0);
//        }
//
//        return equationString;
//    }
